...
using VyinChatSdk.Internal.Domain.Reconnection;
using VyinChatSdk.Internal.Domain.TokenRefresh;
...
namespace VyinChatSdk.Internal.Data.Network
{
    ...
    internal class ConnectionManager : IDisposable
    {
        ...
        private Action<VcUser, VcException> _loginHandler;

        /// <summary>
        /// Current connection state
        /// </summary>
        public VcConnectionState State => _webSocketClient?.State ?? VcConnectionState.Closed;
        ...
        /// Upper layer (VyinChatMain) is responsible for parsing and handling
        /// </summary>
        ...
        private void ForwardSessionDidHaveError(VcException error) => OnSessionDidHaveError?.Invoke(error);

        #endregion

        // Token refresh state
        private bool _isTokenRefreshing;
        private Action<VcUser, VcException> _tokenRefreshCallback;
        ...
        public void Connect(
            ...
            Action<VcUser, VcException> callback)
        {
            if (string.IsNullOrEmpty(userId))
            {
                var error = new VcException(VcErrorCode.InvalidParameter, "userId cannot be null or empty");
            ...
            CancellationToken cancellationToken = default)
        {
            if (!IsConnected)
            {
                throw new VcException(VcErrorCode.WebSocketConnectionClosed, "Not connected to server");
            }
        ...
        private void HandleWebSocketError(VcException error)
        {
        ...
        private void HandleLogiCommand(string payload)
        {
            ...
            if (logi != null)
            {
                ...
                if (logi.IsSuccess())
                {
                    ...
                    if (_loginHandler != null)
                    {
                        var user = new VcUser { UserId = userId };
                ...
                else
                {
                    ...
                    var error = new VcException(VcErrorCode.ErrInvalidSession, "Authentication failed (LOGI error).");
        ...
        public void ReconnectWithToken(string newAccessToken, Action<VcUser, VcException> callback)
        {
            if (string.IsNullOrEmpty(newAccessToken))
            {
                var error = new VcException(VcErrorCode.SessionKeyRefreshFailed, "Access token cannot be null or empty");
            ...
            if (_lastConfig == null)
            {
                var error = new VcException(VcErrorCode.SessionKeyRefreshFailed, "No previous connection config available");
        ...
        private void ClearAllPendingAcks()
        {
            lock (_ackLock)
            {
                ...
                foreach (var ack in pendingAcks)
                {
                    ...
                    {
                        ack.TimeoutCancellation?.Cancel();
                        var error = new VcException(VcErrorCode.WebSocketConnectionClosed, "Connection closed");
        ...
        private void StartAuthTimeout()
        {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    // Handle timeout
                    var error = new VcException(VcErrorCode.LoginTimeout, "Authentication timeout (LOGI not received).");
        ...
        private void AttemptReconnection()
        {
            ...
            var error = new VcException(VcErrorCode.WebSocketConnectionClosed, "Connection lost");
            if (!_reconnectionManager.ShouldAttemptReconnect(error))
            {
        ...
        /// Called by VyinChatMain when app returns to foreground
        /// </summary>
