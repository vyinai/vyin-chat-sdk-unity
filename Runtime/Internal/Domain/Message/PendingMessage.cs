...
namespace VyinChatSdk.Internal.Domain.Message
{
    ...
    public class PendingMessage
    {
        ...
        /// </summary>
        public VcUserMessageCreateParams CreateParams { get; }

        /// <summary>
        /// Current sending status.
        /// </summary>
        public VcSendingStatus Status { get; private set; } = VcSendingStatus.Pending;

        /// <summary>
        /// Error code if failed. Null means no error.
        /// </summary>
        public VcErrorCode? ErrorCode { get; set; }
        ...
        /// </summary>
        public Action<PendingMessage, VcException> OnFailed { get; set; }

        /// <summary>
        /// Callback to invoke on success.
        /// </summary>
        public Action<VcBaseMessage> OnSuccess { get; set; }

        /// <summary>
        /// Event triggered when sending status changes.
        /// Provides the channel URL, base message, and new status.
        /// </summary>
        public Action<string, VcBaseMessage, VcSendingStatus> OnStatusChanged { get; set; }

        /// <summary>Linked base message for status synchronization.</summary>
        public VcBaseMessage BaseMessage { get; }

        /// <summary>
        /// Creates a new pending message.
        /// </summary>
        public PendingMessage(string requestId, VcUserMessageCreateParams createParams, VcBaseMessage baseMessage)
        {
            RequestId = requestId ?? throw new ArgumentNullException(nameof(requestId));
            BaseMessage = baseMessage ?? throw new ArgumentNullException(nameof(baseMessage));
            CreateParams = createParams;
            CreatedAtTicks = DateTime.UtcNow.Ticks;
            Status = VcSendingStatus.Pending;
            SyncToBaseMessage();
        }
        ...
        {
            if (Status.CanTransitionTo(VcSendingStatus.Pending))
            {
                Status = VcSendingStatus.Pending;
                SyncToBaseMessage();
            }
        ...
        {
            if (Status.CanTransitionTo(VcSendingStatus.Succeeded))
            {
                Status = VcSendingStatus.Succeeded;
                SyncToBaseMessage();
            }
        }
        ...
        /// </summary>
        public void MarkAsFailed(VcErrorCode errorCode)
        {
            if (Status.CanTransitionTo(VcSendingStatus.Failed))
            {
                Status = VcSendingStatus.Failed;
                ErrorCode = errorCode;
                SyncToBaseMessage();
            }
        }
        ...
        {
            if (Status.CanTransitionTo(VcSendingStatus.Canceled))
            {
                Status = VcSendingStatus.Canceled;
                SyncToBaseMessage();
            }
        }
