...
using VyinChatSdk.Internal.Data.Message;
...
namespace VyinChatSdk.Internal.Platform.Unity
{
    internal class VyinChatMain
    {
        ...
        private VcBackgroundDisconnectionConfig _backgroundDisconnectionConfig;
        ...
        private IVcSessionHandler _sessionHandler;

        // Connection handlers
        private readonly Dictionary<string, VcConnectionHandler> _connectionHandlers = new();
        ...
        public VyinChatMain()
        {
            ...
            _backgroundDisconnectionConfig = VcBackgroundDisconnectionConfig.Default;
        }
        ...
        private void ConnectInternal(string userId, string authToken, string apiHost, string wsHost, VcUserHandler callback)
        {
            ...
            _messageRepository = new MessageRepositoryImpl(_connectionManager);
            ...
            // Convert VcUserHandler to Action<VcUser, VcException>
            Action<VcUser, VcException> connectionCallback = (user, error) =>
            {
                if (error == null)
                {
                    _currentUser = user;
        ...
        /// </summary>
        private void HandleMessageStatusChanged(string channelUrl, VcBaseMessage message, VcSendingStatus status)
        {
            if (string.IsNullOrEmpty(channelUrl) || message == null)
                return;

            var channel = new VcGroupChannel { ChannelUrl = channelUrl };
        ...
        /// </summary>
        public VcConnectionState GetConnectionState()
        {
            return _connectionManager?.State ?? VcConnectionState.Closed;
        }
        ...
        /// </summary>
        public void SetBackgroundDisconnectionConfig(VcBackgroundDisconnectionConfig config)
        {
            _backgroundDisconnectionConfig = config ?? VcBackgroundDisconnectionConfig.Default;
        ...
        /// </summary>
        public VcBackgroundDisconnectionConfig GetBackgroundDisconnectionConfig()
        {
        ...
        public void AddConnectionHandler(string id, VcConnectionHandler handler)
        {
        ...
        private void NotifyConnectionHandlers(Action<VcConnectionHandler> action)
        {
        ...
        private void HandleMessageReceived(string payload)
        {
            ...
            if (message != null)
            {
                var channel = new VcGroupChannel { ChannelUrl = message.ChannelUrl };
        ...
        /// </summary>
        private VcBaseMessage ParseBroadcastMessage(string payload, string commandType)
        {
            try
            {
        ...
        /// </summary>
        public void SetSessionHandler(IVcSessionHandler handler)
        {
        ...
        private void HandleSessionDidHaveError(VcException error)
        {
            ...
            if (error.ErrorCode == VcErrorCode.ErrInvalidSession ||
                error.ErrorCode == VcErrorCode.ErrInvalidSessionKeyValue)
            {
        ...
        /// </summary>
        internal void SimulateSessionError(VcException error)
        {
        ...
        private void SetupLifecycleMonitoring()
        {
            ...
            var go = new GameObject("VyinChatLifecycleCallbacks");
